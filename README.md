...
- 히어로 배너 — 전체 화면(80vh) 자동재생 동영상, 타이틀 + CTA 링크 오버레이
- 아몬드 컬렉션 탭 필터 (모두보기 · 바디 케어 · 핸드 케어 · 리필) — 클릭 시 상품 목록 동적 렌더링
- API로 전체 상품 조회 후 아몬드 키워드 기준 필터링
- 카테고리 카드 그리드 — 2열(모바일) / 4열(데스크탑) 반응형, 홀수 행 지그재그 오프셋 레이아웃
- 추천 상품 캐러셀
   - 모바일 / 데스크탑 슬라이드셋 분리 렌더링 — API 조회 (`mobile` / `sub1`)
   - 무한 루프 — 양 끝 클론 슬라이드 삽입 후 순간이동 처리
   - 활성 슬라이드 확대 / 비활성 슬라이드 축소 및 흐림 처리
   - 이전·다음 버튼, 슬라이드명 레이블, 카운터 표시
   - 드래그(마우스) · 스와이프(터치) 인터랙션 지원
   - 화면 크기 변경 시 모바일 ↔ 데스크탑 슬라이드셋 자동 전환
- 리필 홍보 영상 섹션 — 모바일/데스크탑 높이 분기 처리, 좌하단 텍스트 오버레이
- JS 모듈 분리: `index.js`, `productTabs.js`, `product-card-list.js`, `categoryCard.js`, `carouselSlide.js`
...
- `slug` 쿼리 파라미터로 페이지 설정 분기 (`hand-care` / `body-care`) — config 파일 기반으로 콘텐츠 전체 주입
- URL `?categoryId=` 파라미터로 카테고리 동적 로딩
- 카테고리 태그 목록 — API로 카테고리 슬러그 맵 조회 후 태그 동적 렌더링, 클릭 시 해당 카테고리 페이지로 이동
- 상품 탭 필터 + 목록 — API로 상품 조회, 탭 클릭 시 config 키워드 기준 필터링 후 상품 카드 동적 렌더링
- 대표 제품 섹션 — config 기반 이미지 · 타이틀 · 설명 · 링크 렌더링
- 카테고리 별 만나보기 — API 슬러그 맵 연동, 카테고리 카드 클릭 시 해당 카테고리 페이지로 이동
- 리추얼 스텝 섹션 — config 기반 3단계 이미지 · 타이틀 · 설명 동적 렌더링
- JS 모듈 분리: `index.js`, `hand-care.js`, `body-care.js`, `productTabs.js`, `product-card-list.js`, `categoryCard.js`, `tag.js`
...
**로그인**
- 아이디 미입력 시 에러 아이콘 + 메시지 표시, 입력 시 에러 초기화
- 비밀번호 값 있을 때 눈 아이콘 표시 — 클릭으로 보기/숨기기 토글
- 로그인 성공 시 `token` · `role` · `member` localStorage 저장
- `role` 기반 이동 분기 — `admin`: 관리자 대시보드 / `user`: 메인 페이지
- 로그인 실패 시 서버 메시지 에러 표시

**회원가입 (공통)**
- 아이디 — 4~20자 형식 검사 + API 중복 확인, 사용 가능·불가 메시지 렌더링
- 비밀번호 — 영문·숫자·특수문자 포함 8자 이상 형식 검사
- 비밀번호 확인 — 실시간 일치 여부 검사
- 이메일 — 형식 검사 
- 각 필드 실시간 유효성 검사 — 체크/에러 아이콘 토글
- 제출 전 전체 필드 순차 검증, 오류 필드 자동 포커스
- 성공 시 로그인 페이지로 이동

**일반 회원가입 추가 필드**
- 주소 · 상세 주소 — 입력 시 체크 아이콘 표시

**관리자 회원가입 추가 필드**
- 관리자 토큰 — API `401`/`403` 에러 시 해당 필드에 에러 표시
  
- JS 모듈 분리: `login/index.js`, `signup/user/index.js`, `signup/admin/index.js`, `createSignupPage.js`, `validation.js`, `ui.js`
