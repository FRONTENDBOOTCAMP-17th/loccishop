...
본인이 작성한 리뷰를 삭제합니다. **삭제 시 리뷰 작성 보상으로 받은 200포인트가 함께 차감됩니다.** 현재 잔여 포인트가 200점 미만이라면 잔여 포인트만큼만 차감되어 음수가 되지 않습니다.
...
| deductedPoint | 실제로 차감된 포인트 수 | 보통 200, 잔액 부족 시 잔액 전액    |
| message       | 안내 메시지             |                                     |
...
> **참고**: 리뷰 작성 시 `+200p`, 삭제 시 `-200p`로 포인트 수지가 정확히 균형을 이룹니다. 포인트 소비 후 리뷰를 삭제하는 경우에도 **계정 포인트가 음수가 되지 않도록** 잔여 포인트까지만 차감합니다.
